...
/* Mesma lista de administradores do admin.html — usada apenas para
   reconhecer, no login do painel da equipe, se quem está entrando
   é um admin e redirecioná-lo automaticamente para admin.html */
const EQ_ADMINS = [
  { email: "lv196814@gmail.com", senha: "LVxV7!_0917", nome: "Luan Vinícius" }
];
...
let eqUploadFiles = [];         // arquivos selecionados no modal de upload
...
async function verificarLoginEquipe() {
  ...
  // Verifica primeiro se é um administrador (mesma lista do admin.html).
  // Se for, redireciona direto para o painel de gestão em admin.html.
  const admin = EQ_ADMINS.find(a => a.email.toLowerCase() === usuario.toLowerCase() && a.senha === senha);
  if (admin) {
    sessionStorage.setItem("lumina_admin", "ok");
    sessionStorage.setItem("lumina_adm_nome", admin.nome);
...
function montarCardServicoAndamento(s) {
  ...
  if (s.status === "em_andamento") {
    acaoHtml = `<button class="btn primary" style="width:100%" onclick="abrirModalUpload(${s.id})">Enviar Arquivos</button>`;
    ...
    acaoHtml = `
      ...
      <button class="btn primary" style="width:100%" onclick="abrirModalUpload(${s.id})">Reenviar Arquivos</button>`;
  }
...
function abrirModalUpload(servicoId) {
  ...
  document.getElementById("uploadModalList").innerHTML = "";
...
document.addEventListener("DOMContentLoaded", () => {
  const input = document.getElementById("uploadModalInput");
  if (input) {
    input.addEventListener("change", (e) => {
      Array.from(e.target.files).forEach((f) => eqUploadFiles.push(f));
      renderizarListaUpload();
      input.value = "";
    });
  }
});

function renderizarListaUpload() {
  const list = document.getElementById("uploadModalList");
  list.innerHTML = eqUploadFiles.map((f, i) => `
    <div class="eq-upload-item">
      <span>📄 ${escapeHtml(f.name)} (${(f.size / 1024).toFixed(0)} KB)</span>
      <button onclick="removerArquivoUpload(${i})">✕</button>
    </div>`).join("");
}

function removerArquivoUpload(i) {
  eqUploadFiles.splice(i, 1);
  renderizarListaUpload();
}

async function confirmarEnvioArquivos() {
  if (eqUploadFiles.length === 0) { mostrarToast("Selecione ao menos um arquivo.", "err"); return; }
  ...
  try {
    const urls = [];
    for (const file of eqUploadFiles) {
      const nomeArq = `servico_${eqUploadServicoId}/${Date.now()}_${file.name}`;
      const { error: upErr } = await supaEquipe.storage.from("equipe_arquivos").upload(nomeArq, file);
      if (upErr) throw upErr;
      const { data: urlData } = supaEquipe.storage.from("equipe_arquivos").getPublicUrl(nomeArq);
      urls.push({ nome: file.name, url: urlData.publicUrl });
    ...
    mostrarToast("Arquivos enviados! Aguardando aprovação.", "ok");
    fecharModalUpload();
    carregarServicosAndamento();
  } catch (err) {
