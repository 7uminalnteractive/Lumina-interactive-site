...
async function enviarMensagemDm() {
  ...
  // guarda tudo que precisamos ANTES de limpar a UI, pra não perder nada
  // se o envio falhar no meio do caminho
  const respostaAId = dmRespondendoA ? dmRespondendoA.id : null;
  const conversaId  = dmConversaAtivaId;

  input.value = "";
  cancelarResposta();
  cancelarImagemStaged();

  const sendBtn = document.querySelector("#dmForm .dm-send-btn");
...
function abrirEditor(file, callback) {
  ...
  reader.onload = e => {
    ...
    img.onload = () => {
      ...
      // espera dois frames: o primeiro garante que o overlay já está
      // visível no DOM, o segundo garante que o layout (inclusive ajustes
      // de viewport do navegador mobile, tipo esconder a barra de endereço)
      // já se estabilizou antes de medir o wrap e desenhar o canvas
...
function ieRender() {
  ...
  canvas.width  = rW;
  canvas.height = rH;
  // fixa o tamanho VISUAL do canvas igual ao tamanho em pixels internos.
  // Sem isso, o CSS (max-width/max-height:100%) pode exibir o canvas em um
  // tamanho diferente do calculado aqui, e todo o posicionamento do
  // retângulo de corte (que usa esses valores) fica dessincronizado da
...
function ieUpdateUI() {
  ...
  // o canvas fica centralizado dentro do wrap (flex center), então há um
  // offset entre a origem do wrap e a origem real do canvas nos dois eixos.
  // O retângulo de crop e as sombras são posicionados relativos ao WRAP
  // (pai com position:relative), então esse offset precisa ser somado —
  // sem isso, o retângulo aparecia deslocado e não refletia a área real
  // que seria cortada.
  const offX = (wW - rW) / 2;
  const offY = (wH - rH) / 2;

  const px = offX + Math.round(ie.cx * rW);
  const py = offY + Math.round(ie.cy * rH);
  ...
  shade("ieShadeL", 0,        py,      px,        ph);
  shade("ieShadeR", px+pw,    py,      wW-px-pw,  ph);
}
...
function confirmarEditor() {
  ...
  const rot    = ie.rotation;
  const rotated= rot === 90 || rot === 270;
  // dimensões lógicas após rotação — usa a resolução ORIGINAL da imagem
  // (antes o crop usava ie.renderW/renderH, o tamanho do preview na tela,
  // o que fazia a imagem final sair em baixa resolução)
  const logW   = rotated ? img.height : img.width;
  const logH   = rotated ? img.width  : img.height;

  // área de crop em pixels reais da imagem lógica, em resolução total
  ...
  // canvas intermediário na resolução ORIGINAL com a imagem já rotacionada/flipada
  ...
  // recorta a área certa do canvas intermediário, em resolução total
  ctx.drawImage(mid, cropX, cropY, cropW, cropH, 0, 0, out.width, out.height);

  // se o resultado for maior que um teto razoável, reduz proporcionalmente.
  // Evita arquivos enormes (fotos de celular passam fácil de 4000px) que
  // demoram demais ou falham no upload — 2048px já é mais que suficiente
  // pra qualquer tela.
  const MAX_DIM = 2048;
  let finalCanvas = out;
  if (out.width > MAX_DIM || out.height > MAX_DIM) {
    const s = Math.min(MAX_DIM / out.width, MAX_DIM / out.height);
    const small = document.createElement("canvas");
    small.width  = Math.round(out.width * s);
    small.height = Math.round(out.height * s);
  ...
  finalCanvas.toBlob(blob => {
    ...
    const callback = ie.callback; // guarda antes do cancelarEditor() limpar ie.callback
    cancelarEditor();
