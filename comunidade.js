...
function previewImagemComposer() {
  ...
  abrirEditor(file, (blob) => {
    const editado = new File([blob], file.name, { type: "image/jpeg" });
    // substitui o arquivo no input via DataTransfer
    const dt = new DataTransfer();
    dt.items.add(editado);
    document.getElementById("imgInput-post").files = dt.files;
...
function stageImagemDm() {
  ...
  abrirEditor(file, (blob) => {
    const editado = new File([blob], file.name, { type: "image/jpeg" });
    dmImagemStagedFile = editado;
    const reader = new FileReader();
    reader.onload = (e) => {
      document.getElementById("dmImgStagedPreview").src = e.target.result;
...
/* ═══════════════════════════════════════════
   EDITOR DE IMAGEM (crop + flip + rotate)
   ═══════════════════════════════════════════ */
const imgEditor = {
  img: null,           // HTMLImageElement original
  callback: null,      // função chamada com o Blob final
  flipH: false,
  flipV: false,
  rotation: 0,         // múltiplos de 90
  cropMode: true,
  // crop em coordenadas da imagem original (antes de flip/rotate)
  crop: { x:0, y:0, w:0, h:0 },
  // drag do retângulo de crop
  dragging: false,
  dragStart: null,
  dragType: null,      // 'move' | 'new'
  canvasRect: null,
  scale: 1,            // pixels da imagem por pixel do canvas
};

function abrirEditor(file, callback) {
  imgEditor.callback = callback;
  imgEditor.flipH = false;
  imgEditor.flipV = false;
  imgEditor.rotation = 0;
  imgEditor.cropMode = true;
  ...
  reader.onload = (e) => {
    ...
    img.onload = () => {
      imgEditor.img = img;
      imgEditor.crop = { x:0, y:0, w:img.width, h:img.height };
      document.getElementById("editorBtnCrop").classList.add("active");
      document.getElementById("imgEditorOverlay").classList.add("open");
      editorDraw();
      editorPositionCropRect();
    };
    img.src = e.target.result;
  };
  reader.readAsDataURL(file);
...
function editorDraw() {
  ...
  const rot    = imgEditor.rotation;

  // tamanho lógico da imagem após rotação
  ...
  const maxH = Math.min(window.innerHeight * 0.52, maxW * (logH / logW));
  ...
  if (imgEditor.flipH) ctx.scale(-1, 1);
  if (imgEditor.flipV) ctx.scale(1, -1);
  ctx.drawImage(img, -img.width * scale / 2, -img.height * scale / 2, img.width * scale, img.height * scale);
  ctx.restore();
}

function editorPositionCropRect() {
  if (!imgEditor.cropMode) {
    document.getElementById("imgEditorCropRect").style.display = "none";
    document.getElementById("imgEditorCropMask").style.display = "none";
    return;
  }
  const canvas  = document.getElementById("imgEditorCanvas");
  const wrap    = document.getElementById("imgEditorCanvasWrap");
  const rect    = document.getElementById("imgEditorCropRect");
  const mask    = document.getElementById("imgEditorCropMask");
  const img     = imgEditor.img;
  const rot     = imgEditor.rotation;
  const rotated = (rot % 180 !== 0);
  const logW    = rotated ? img.height : img.width;
  const logH    = rotated ? img.width  : img.height;
  const scale   = imgEditor.scale;

  // crop em coord da imagem rotacionada
  const cx = imgEditor.crop.x / img.width  * logW * scale;
  const cy = imgEditor.crop.y / img.height * logH * scale;
  const cw = imgEditor.crop.w / img.width  * logW * scale;
  const ch = imgEditor.crop.h / img.height * logH * scale;

  rect.style.cssText = `display:block;left:${cx}px;top:${cy}px;width:${cw}px;height:${ch}px`;
  mask.style.display = "block";
}

function editorFlip(dir) {
  if (dir === 'h') imgEditor.flipH = !imgEditor.flipH;
  else imgEditor.flipV = !imgEditor.flipV;
  editorDraw();
  editorPositionCropRect();
}

function editorRotate(deg) {
  imgEditor.rotation = ((imgEditor.rotation + deg) % 360 + 360) % 360;
  // resetar crop pra imagem inteira após rotação
  imgEditor.crop = { x:0, y:0, w:imgEditor.img.width, h:imgEditor.img.height };
  editorDraw();
  editorPositionCropRect();
}
...
function editorToggleCrop() {
  ...
  document.getElementById("editorBtnCrop").classList.toggle("active", imgEditor.cropMode);
  editorPositionCropRect();
}

// ── drag pra desenhar crop ──
const _wrap = () => document.getElementById("imgEditorCanvasWrap");

function editorPointerDown(e) {
  if (!imgEditor.cropMode) return;
  imgEditor.dragging = true;
  imgEditor.canvasRect = _wrap().getBoundingClientRect();
  const p = editorEventPos(e);
  imgEditor.dragStart = p;
  imgEditor.dragType = 'new';
  e.preventDefault();
}
function editorPointerMove(e) {
  if (!imgEditor.dragging || !imgEditor.cropMode) return;
  const p   = editorEventPos(e);
  const s   = imgEditor.dragStart;
  const img = imgEditor.img;
  const rot = imgEditor.rotation;
  const rotated = (rot % 180 !== 0);
  const logW = rotated ? img.height : img.width;
  ...
  const x1 = Math.max(0, Math.min(s.x, p.x));
  const y1 = Math.max(0, Math.min(s.y, p.y));
  const x2 = Math.min(logW * sc, Math.max(s.x, p.x));
  const y2 = Math.min(logH * sc, Math.max(s.y, p.y));

  imgEditor.crop = {
    x: x1 / sc / logW * img.width,
    y: y1 / sc / logH * img.height,
    w: (x2 - x1) / sc / logW * img.width,
    h: (y2 - y1) / sc / logH * img.height,
  };
  editorPositionCropRect();
  e.preventDefault();
}
function editorPointerUp() { imgEditor.dragging = false; }

function editorEventPos(e) {
  const r = imgEditor.canvasRect;
  const src = e.touches ? e.touches[0] : e;
  return { x: src.clientX - r.left, y: src.clientY - r.top };
}

// registra eventos de drag no wrap
document.addEventListener("DOMContentLoaded", () => {
  const w = document.getElementById("imgEditorCanvasWrap");
  w.addEventListener("mousedown",  editorPointerDown);
  w.addEventListener("mousemove",  editorPointerMove);
  w.addEventListener("mouseup",    editorPointerUp);
  w.addEventListener("touchstart", editorPointerDown, { passive:false });
  w.addEventListener("touchmove",  editorPointerMove, { passive:false });
  w.addEventListener("touchend",   editorPointerUp);
});

function cancelarEditor() {
  document.getElementById("imgEditorOverlay").classList.remove("open");
  imgEditor.callback = null;
}
...
function confirmarEditor() {
  ...
  // canvas de saída no tamanho do crop
  const out = document.createElement("canvas");
  let cw = imgEditor.crop.w || img.width;
  let ch = imgEditor.crop.h || img.height;
  // converter crop de coord original pra coord lógica (após rotação)
  const scaleX = logW / img.width;
  const scaleY = logH / img.height;
  out.width  = Math.round(cw * scaleX);
  out.height = Math.round(ch * scaleY);

  const ctx = out.getContext("2d");
  ctx.save();
  // move a origem pro centro do canvas de saída
  ctx.translate(out.width / 2, out.height / 2);
  ctx.rotate((rot * Math.PI) / 180);
  if (imgEditor.flipH) ctx.scale(-1, 1);
  if (imgEditor.flipV) ctx.scale(1, -1);
  // desenha a imagem deslocada pelo crop
  const cropCx = imgEditor.crop.x + cw / 2;
  const cropCy = imgEditor.crop.y + ch / 2;
  ctx.drawImage(img, -cropCx, -cropCy, img.width, img.height);
  ctx.restore();

  out.toBlob((blob) => {
    document.getElementById("imgEditorOverlay").classList.remove("open");
    if (imgEditor.callback) imgEditor.callback(blob);
  }, "image/jpeg", 0.92);
}
