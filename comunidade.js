...
function confirmarEditor() {
  ...
  const rot    = ie.rotation;
  const rotated= rot === 90 || rot === 270;
  // dimensões lógicas da imagem após rotação (em pixels reais)
  const imgW   = rotated ? img.height : img.width;
  const imgH   = rotated ? img.width  : img.height;
  const scaleX = imgW / ie.canvasW;
  const scaleY = imgH / ie.canvasH;

  // crop em pixels da imagem real (alta resolução)
  const cropX = Math.round((ie.cx - ie.canvasX) * scaleX);
  const cropY = Math.round((ie.cy - ie.canvasY) * scaleY);
  const cropW = Math.round(ie.cw * scaleX);
  const cropH = Math.round(ie.ch * scaleY);

  // canvas de saída na resolução REAL (não na do preview)
  const out = document.createElement("canvas");
  out.width  = cropW;
  out.height = cropH;
  const ctx  = out.getContext("2d");

  // desenha a imagem original com rotação/flip, cortando a área certa
  ctx.save();
  ...
  // centro do crop nas coordenadas da imagem original (sem rotação)
  let originCtrX, originCtrY;
  if (rot === 0)   { originCtrX = cropX + cropW/2; originCtrY = cropY + cropH/2; }
  else if (rot===90) { originCtrX = cropY + cropH/2; originCtrY = imgW - cropX - cropW/2; }
  else if (rot===180){ originCtrX = imgW - cropX - cropW/2; originCtrY = imgH - cropY - cropH/2; }
  else              { originCtrX = imgH - cropY - cropH/2; originCtrY = cropX + cropW/2; }

  ctx.drawImage(img, -originCtrX, -originCtrY, img.width, img.height);
  ctx.restore();
