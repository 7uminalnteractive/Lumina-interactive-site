...
async function abrirThreadDm(conversaId, outroPerfil) {
  ...
  msgsEl.innerHTML = "";
  (mensagens || []).filter(m => !ocultasIds.has(m.id)).forEach(m => adicionarMensagemDm(m));
  scrollDmParaFim();
...
function atualizarMensagemDmNaTela(m) {
  ...
  if (!div) return;
  div.classList.toggle("apagada", !!m.apagada_para_todos);
  const bubble = div.querySelector(".dm-bubble");
  bubble.innerHTML = montarConteudoBolhaDm(m);
  bubble.setAttribute("onclick", m.apagada_para_todos ? "" : `abrirMenuMensagem(${m.id})`);
}
...
function abrirMenuMensagem(mensagemId) {
  ...
  preview.style.display = (m.texto || m.imagem_url || m.sticker_url) ? "block" : "none";

  document.getElementById("dmMsgMenuApagarTodos").style.display = minha ? "flex" : "none";
...
async function abrirPerfilPublico(usuarioId) {
  ...
  const { data: perfil, error } = await supabaseClient
    .from("perfis").select("id, nome, id_online, foto_url, bio, verificado, email").eq("id", usuarioId).maybeSingle();
