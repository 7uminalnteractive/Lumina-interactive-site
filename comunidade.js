...
async function initComunidade() {
  ...
  if (session?.user) {
    ...
    const { data: perfil } = await supabaseClient
      .from("perfis").select("nome, id_online, foto_url, bio, verificado").eq("id", session.user.id).maybeSingle();
    meuPerfil = perfil || {};
...
async function checarAssinantePlus(email) {
  ...
  const { data } = await supabaseClient.from("pedidos").select("*")
    .eq("email", email).eq("produto", "Lumina Plus").in("status", ["aprovado", "ativa"])
