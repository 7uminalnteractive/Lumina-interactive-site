...
const ie = {
  ...
  // crop em pixels do canvas renderizado
  cx: 0, cy: 0, cw: 0, ch: 0,
  // limites do canvas renderizado dentro do wrap
...
function abrirEditor(file, callback) {
  ...
  reader.onload = e => {
    ...
    img.onload = () => {
      ...
      requestAnimationFrame(() => { ieLayout(); ieFullCrop(); ieUpdateUI(); });
    };
    img.src = e.target.result;
  };
  reader.readAsDataURL(file);
...
/* desenha a imagem no canvas com flip/rotate */
function ieLayout() {
  ...
  const wrapH = wrap.clientHeight || window.innerHeight * 0.6;
  ...
  // posição do canvas dentro do wrap (centralizado)
  const rect = canvas.getBoundingClientRect();
  const wrect= wrap.getBoundingClientRect();
  ie.canvasX = rect.left - wrect.left;
  ie.canvasY = rect.top  - wrect.top;
  ie.canvasW = cw;
  ie.canvasH = ch;
}
...
/* atualiza o retângulo visual e as sombras */
function ieUpdateUI() {
  const rect  = document.getElementById("imgEditorCropRect");
  const wrap  = document.getElementById("imgEditorCanvasWrap");
  const wW    = wrap.clientWidth;
  const wH    = wrap.clientHeight;
  const { cx, cy, cw, ch } = ie;

  rect.style.cssText = `left:${cx}px;top:${cy}px;width:${cw}px;height:${ch}px`;

  // 4 sombras: topo, baixo, esquerda, direita
  const s = (id, l, t, w, h) => {
    const el = document.getElementById(id);
    el.style.cssText = `left:${l}px;top:${t}px;width:${w}px;height:${h}px`;
  };
  s("ieShadeT", 0,       0,      wW,        cy);
  s("ieShadeB", 0,       cy+ch,  wW,        wH - cy - ch);
  s("ieShadeL", 0,       cy,     cx,        ch);
  s("ieShadeR", cx+cw,   cy,     wW-cx-cw,  ch);
}

/* ── eventos de drag nas alças ── */
...
function iePointerMove(e) {
  ...
  const p  = iePos(e);
  const dx = p.x - ie.dragStartX;
  const dy = p.y - ie.dragStartY;
  let { cx, cy, cw, ch } = ie.dragStartCrop;
  const h  = ie.activeHandle;
  const minX = ie.canvasX, minY = ie.canvasY;
  const maxX = ie.canvasX + ie.canvasW, maxY = ie.canvasY + ie.canvasH;
  const MIN  = ie.MIN;

  if (h === "move") {
    cx = Math.max(minX, Math.min(maxX - cw, cx + dx));
    cy = Math.max(minY, Math.min(maxY - ch, cy + dy));
  } else {
    if (h.includes("l")) { const nx = Math.min(cx + cw - MIN, Math.max(minX, cx + dx)); cw += cx - nx; cx = nx; }
    if (h.includes("r")) { cw = Math.max(MIN, Math.min(maxX - cx, cw + dx)); }
    if (h.includes("t")) { const ny = Math.min(cy + ch - MIN, Math.max(minY, cy + dy)); ch += cy - ny; cy = ny; }
    if (h.includes("b")) { ch = Math.max(MIN, Math.min(maxY - cy, ch + dy)); }
    if (h === "lm")      { const nx = Math.min(cx + cw - MIN, Math.max(minX, cx + dx)); cw += cx - nx; cx = nx; }
    if (h === "rm")      { cw = Math.max(MIN, Math.min(maxX - cx, cw + dx)); }
    if (h === "tm")      { const ny = Math.min(cy + ch - MIN, Math.max(minY, cy + dy)); ch += cy - ny; cy = ny; }
    if (h === "bm")      { ch = Math.max(MIN, Math.min(maxY - cy, ch + dy)); }
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  w.addEventListener("mousedown",  iePointerDown);
  window.addEventListener("mousemove",  iePointerMove);
  window.addEventListener("mouseup",    iePointerUp);
  w.addEventListener("touchstart", iePointerDown,  { passive:false });
...
function editorRotate(deg) {
  ...
  ieLayout(); ieFullCrop(); ieUpdateUI();
}
...
function confirmarEditor() {
  ...
  const imgW   = rotated ? img.height : img.width;
  const imgH   = rotated ? img.width  : img.height;
  const scaleX = imgW / ie.canvasW;
  const scaleY = imgH / ie.canvasH;

  // crop em pixels da imagem lógica (após rotação)
  const cropX = (ie.cx - ie.canvasX) * scaleX;
  const cropY = (ie.cy - ie.canvasY) * scaleY;
  const cropW = ie.cw * scaleX;
  const cropH = ie.ch * scaleY;
  ...
  if (ie.flipH) ctx.scale(-1,  1);
  if (ie.flipV) ctx.scale( 1, -1);
  // offset: centro do crop na coord da imagem original
  const ctrX = (cropX + cropW / 2) / imgW * (rotated ? img.height : img.width);
  const ctrY = (cropY + cropH / 2) / imgH * (rotated ? img.width  : img.height);
  const drawW = img.width  * (out.width  / cropW / (rotated ? img.height/img.width  : 1));
  const drawH = img.height * (out.height / cropH / (rotated ? img.width /img.height : 1));
  ctx.drawImage(img,
    -(rotated ? ctrY : ctrX) * (out.width  / cropW),
    -(rotated ? ctrX : ctrY) * (out.height / cropH),
    img.width  * (out.width  / cropW),
    img.height * (out.height / cropH)
  );
  ctx.restore();
